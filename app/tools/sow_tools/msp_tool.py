...
class GetMspInput(BaseModel):
    client_id: str = Field(..., description="The Client ID (User ID) to fetch MSP details for.")

@tool(args_schema=GetMspInput)
def get_msp(client_id: str) -> str:
    """Retrieves MSP details for a given client from the QA environment."""
    ...
    try:
        ...
        return f"Error fetching MSP details: {str(e)}"
